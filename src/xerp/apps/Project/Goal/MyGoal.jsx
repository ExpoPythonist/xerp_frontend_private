...
import { allGoalList } from '../../../redux/actions'
...
class MyGoal extends React.Component {
  ...
  allGoals = async (params) => {
    try {
      await this.props.allGoalList(params);
    } catch (e) {
      console.log(e)
    }
  }
                    ...
                    renderItem={item => (
                      <List.Item>
                        <Col lg={4} md={6} sm={12}>
                          <List.Item.Meta
                            avatar={
                              <Avatar size="large" icon="snippets" className="d-flex align-items-center justify-content-center bg-white" />
                            }
                            title={
...
const mapDispatchToProps = dispatch => ({
  allGoalList: (params) => dispatch(allGoalList(params))
})
